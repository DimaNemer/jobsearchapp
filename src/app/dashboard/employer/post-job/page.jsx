...
export default function PostJobPage() {
  ...
  console.log("📋 Post Job Page - Job ID from URL:", jobId);
    ...
    async function loadJob() {
      try {
        ...
        if (!res.ok) {
          ...
          alert(`Failed to load job: ${errorData.error || "Unknown error"}`);
          return;
        }
        ...
        alert("Could not load job. Check console for details.");
      } finally {
        setInitialLoading(false);
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.log("📤 Submitting job:", { isEdit, jobId, payload });
